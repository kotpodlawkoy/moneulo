...
public abstract class CellController : MonoBehaviour
{
    ...
    void ApplyAction(CreatureAction a)
    {
        ...
        {
            if (a.Run) _movement.StartRunning();
            else       _movement.StopRunning();
        ...
        if (a.Heal) _health.Heal(_energy);
        if (a.Breed && _breeder != null && _breeder.CanBreed) _breeder.Breed();
        if (a.Grow  && _growable != null && _growable.CanGrow) _growable.Grow();

        if (_signal != null) _signal.SetSignal(a.Signal);
    }
